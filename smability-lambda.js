  ...
  try {
    ...
    for (const record of records) {
      ...
      const hourlyData = {
        ...
        // 🛡️ FIX CO: Convertimos ppb a ppm dividiendo entre 1000
        co: (record.co_avg !== undefined && record.co_avg !== null) ? (record.co_avg / 1000) : null,
...
async function fetchBioBoxHourlyData(connection, tokenId) {
  try {
    ...
    const hourlyData = {
      ...
      // 🛡️ FIX CO: Convertimos ppb a ppm dividiendo entre 1000
      co: rawCo !== null ? (rawCo / 1000) : null,
      co_8hr: rawCo8hr !== null ? (rawCo8hr / 1000) : null,
...
function transformSensorDataToReadings(device, sensor, sensorData) {
  ...
  for (const dataPoint of sensorData) {
    ...
    if (parameterId) {
      ...
      // 🛡️ FIX CO: Convertimos ppb a ppm dividiendo entre 1000
      if (parameterId === 'co' && !isNaN(finalValue)) {
        finalValue = finalValue / 1000;
      }

      readings.push({
        // Use station_id instead of name
        station_id: device.station_id || device.name, 
